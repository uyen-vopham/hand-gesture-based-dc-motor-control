...
unsigned long last_uart = 0U;
 
uint8_t button,dir = 0;
float setpoint, motor, kp, ki, kd = 0.0f; //global variablle used to send to it.c for PID program
...
float sp_rx, motor_rx, dir_rx, kp_rx, ki_rx, kd_rx = 0.0f; int  button_rx = 0;//data nhan ve tu python
...
int main(void)
  {	
	...
	{	
			...
			LCD_SetCursor(0,7); I2C_LCD_Puts("20 RMP");
			LCD_SetCursor(1,1);I2C_LCD_Puts("MOTOR: ");
			LCD_SetCursor(1,7); I2C_LCD_Puts("FORWARD");
...
//			else
//			{
//				 count_reset = 0; // N?u không có ng?t, gán count_reset b?ng 0
//					ledBTN_RESET(0);
//			}
			...
			dir == 1 ? (GPIO_SetBits(GPIOA, GPIO_Pin_9)) : (GPIO_ResetBits(GPIOA, GPIO_Pin_9));
...
//			
			...
			//
		//	button_stm == 1 ? (ledBNT_START(1), ledBTN_STOP(0) ) : (ledBTN_STOP(1),ledBNT_START(0));
		...
		if( (unsigned long) (millis() - last_uart) >= 500) 
		{

			char JSON[20];char Str_S1[size_Json];char Str_S2[size_Json];
			int i;
			for(i=0; i<50; i++)
			{
				Str_S1[i] = 0;Str_S2[i] = 0; JSON[i] = 0;
			}

			sprintf(Str_S1, "%.2f", speed);sprintf(Str_S2, "%d", button_stm);
			
			strcat(JSON,Str_S1); strcat(JSON,",");strcat(JSON,Str_S2);strcat(JSON,"\r\n");
...
void USART1_IRQHandler(void)
{
	if(USART_GetITStatus(USART1, USART_IT_RXNE) ==SET)
		...
		if (data_rx != '\n')
			...
			{
				uint16_t uyen =	sscanf(rx_buffer, "%f,%d,%f,%f,%f,%f,%f", &sp_rx, &button_rx, &motor_rx, &dir_rx, &kp_rx, &ki_rx, &kd_rx)==7;			
